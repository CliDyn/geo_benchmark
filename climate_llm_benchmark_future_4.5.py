...
def find_latest_intermediate_file(resolution: str, simple_mode: bool = False) -> Optional[str]:
    ...
    pattern = f"results/climate_results_intermediate_*{mode_suffix}_scenario_SSP2-4.5.json"
    ...
    for file_path in intermediate_files:
        try:
            # Extract number from filename like "climate_results_intermediate_1840_simple_scenario_SSP2-4.5.json"
            filename = Path(file_path).stem
            if simple_mode:
                number_part = filename.replace("climate_results_intermediate_", "").replace("_simple_scenario_SSP2-4.5", "")
            else:
                number_part = filename.replace("climate_results_intermediate_", "").replace("_scenario_SSP2-4.5", "")
...
def process_climate_benchmark(config: Dict):
    ...
    for i, point_data in enumerate(land_points[start_index:], start=start_index):
        ...
        if (i + 1) % save_interval == 0:
            ...
            intermediate_file = f"{results_dir}/climate_results_intermediate_{i+1}_{clean_model_name}{mode_suffix}_scenario_SSP2-4.5.json"
...
def main():
    ...
    try:
        ...
        clean_model_name = model_name.replace(":", "_").replace("/", "_").replace(".", "_")
        results_dir = get_config_value(config, 'output.results_dir', 'results')
        output_file = f"{results_dir}/climate_results_{resolution}deg_r{num_repeats}_{clean_model_name}{mode_suffix}_scenario_SSP2-4.5.json"
        # Create results directory if it doesn't exist
