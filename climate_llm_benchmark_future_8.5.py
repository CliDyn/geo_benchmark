...
def create_climate_prompt(simple_mode=False, month="July"):
    ...
    if simple_mode:
        prompt_template = f"""You are a climate data expert. Given the location below, provide your best estimate of the mean {month} near-surface air temperature at 2 m (°C) for the period 2070-2099 under the IPCC AR6 scenario SSP5-8.5.

Background: SSP5-8.5 (IPCC AR6) pairs the "Fossil-Fueled Development" pathway (SSP5) with very high greenhouse-gas emissions, yielding ~8.5 W m⁻² effective radiative forcing by 2100; it is the upper-end scenario and broadly continuous with CMIP5 RCP8.5.
...
Example: 25.4"""
        ...
        prompt_template = f"""You are a climate data expert. Given the location below, provide your best estimate of climatological mean values for temperature and precipitation for the period 2070-2099 under the IPCC AR6 scenario SSP5-8.5.
...
def process_climate_benchmark(config: Dict, mesh_file: str = None):
    ...
    print(f"Scenario: SSP5-8.5 (2070-2099)")
...
def main():
    ...
    if resume:
        ...
        results_dir = get_config_value(config, 'output.results_dir', 'results')
        expected_output_file = f"{results_dir}/climate_results_{temp_resolution}deg_r{num_repeats}_{temp_clean_model_name}{temp_chunk_suffix}{temp_mode_suffix}_scenario_SSP5-8.5.json"
