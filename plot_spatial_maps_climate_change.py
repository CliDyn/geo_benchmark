...
def create_map_plot(mapping_data, field_name, resolution, colormap='viridis', 
    ...
    # Check field types for climate change data
    is_temp_field = (('temp' in field_name.lower() or 'era5_change_signal' in field_name.lower()) and
    ...
    # Create pcolormesh plot with appropriate transform
    # Mask NaN values to prevent coloring ocean areas
    data_grid_masked = np.ma.masked_invalid(data_grid)

    # Make sure the map background isn't tinted by collections
    ax.set_facecolor('white')
    if hasattr(fig, 'patch'):
        fig.patch.set_facecolor('white')

    # Draw the data WITHOUT creating a global "outside" polygon
    if cartopy_mode:
        contour = ax.pcolormesh(
            lon_grid, lat_grid, data_grid_masked,
    ...
    if is_rmse_field:
        ...
        # Use different labels for temperature change vs absolute temperature
        if 'era5_change_signal' in field_name.lower() or 'change' in field_name.lower():
            cbar.set_label('Temperature Change (°C)', fontsize=font_size_label, fontfamily=font_family)
        else:
            cbar.set_label('Temperature Change (°C)', fontsize=font_size_label, fontfamily=font_family)
        # Set nice rounded ticks for temperature colorbar - every 1 degree for change data
        if temp_range is not None:
            temp_ticks = np.arange(temp_range[0], temp_range[1] + 0.1, 1)  # Every 1 degree: -2, -1, 0, 1, 2, 3, 4, 5
...
def create_all_spatial_maps(results_data, resolution, output_dir='png', temp_range=(-13, 40), temp_colormap='thermal', bias_range=(-7, 7), bias_colormap='RdBu_r', rmse_range=(0, 10), rmse_colormap='viridis_r', font_family='Helvetica', font_size_label=10, font_size_tick=8, show_axes=True, show_frame=True, use_cartopy=False, projection='PlateCarree', central_longitude=0, color_levels=20):
    ...
    maps_config = {
        ...
        'era5_change_signal': ('(Individual Points)', 'coolwarm'),
        ...
        if 'temp' in field_name and 'mean' in field_name:
            ...
            if values:
                ...
                if 'neighborhood' in field_name:
                    ...
                    point_field = 'llm_temp_mean' if 'llm' in field_name else 'era5_change_signal'
...
def main():
    ...
    TEMP_MIN = 0  # Minimum temperature in °C
    TEMP_MAX = 2   # Maximum temperature in °C
